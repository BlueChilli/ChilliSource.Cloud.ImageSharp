...
namespace ChilliSource.Cloud.ImageSharp
{
    ...
    //    {
    //        var quality = GetQuality(commands, parser, culture);

    //        if (quality != null)
    //        {
    //            GetHttpContext().Items[ImageQualityKey] = GetQuality(commands, parser, culture);
    //        }
    ...
    //    private static int? GetQuality(IDictionary<string, string> commands, CommandParser parser, CultureInfo culture)
    //    {
    //        var value = commands.GetValueOrDefault(Quality);
    //        return String.IsNullOrEmpty(value) ? (int?)null : parser.ParseValue<int>(value, culture);
    //    }
